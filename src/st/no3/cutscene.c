...
static void CutsceneUnk1(void) {
    ...
    // Note that these two lines are "= 2"
    // for all other cutscenes (so far)!
    g_Dialogue.nextCharX = g_Dialogue.nextLineX =  0x182;
...
#ifdef VERSION_PSP
    memset(D_pspeu_09299478, 0, sizeof(D_pspeu_09299478));
#endif
}
        ...
        while (1) {
            ...
            nextChar = *g_Dialogue.scriptCur++;
            {
                ...
                case CSOP_NEXT_DIALOG:
                    ...
                    for (prim = g_Dialogue.prim[0], i = 0; i < PRIM_COUNT; i++) {
                        prim->drawMode = DRAW_HIDE;
                        prim = prim->next;
                    }
